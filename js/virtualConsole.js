...
window.addEventListener('error', (error) => {
  if( error.error )
    if(error.error.stack == undefined)
      console.error('fake', error.message, error.filename+':'+error.lineno )
    else
...
function createVirtualConsole(){
  ...
  consoleWrap.setAttribute("id","consoleWrap");
  ...
  close.addEventListener('click', (e) => {
    ...
    if( close.textContent == 'close' ){
      ...
      // consoleWrap.style.height = execute.offsetHeight <= 100 ? '100%' : height + 'px';
      consoleWrap.style.height = '100%';
...
var log = function(){
  ...
  if(document.getElementById("consoleWrap"))
    document.getElementById("consoleWrap").scrollTop = document.getElementById("consoleWrap").scrollHeight;
...
var error = function( type, ...args ){
  ...
  var span = document.createElement('span');
  span.textContent = args[0].match(/\w*\.js[:\d]*/m);
  if(span.textContent == '') span.textContent = args[1].match(/\w*\.js[:\d]*/m);
  span.style.cssText = 'float:right; color:#555';
  p.insertBefore(span, p.firstChild)
